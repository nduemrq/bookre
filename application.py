...
def api(isbn):
    book = db.execute("SELECT title, author, year, isbn, id FROM book WHERE isbn = :isbn", {"isbn": isbn}).fetchone()

    avgScore = db.execute("SELECT avg(score) FROM review WHERE id_book = :id_book", {"id_book": book[4]}).fetchall()
           
    count = db.execute("SELECT count(score) FROM review WHERE id_book = :id_book", {"id_book": book[4]}).fetchall()
    ...
    return jsonify({
        ...
        "year": str(book[2]),
        "isbn": str(book[3]),
        "review_count": str(float(avgScore[0][0])),
        "average_score": str(count[0][0])
    })